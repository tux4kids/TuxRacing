...
require(["multiplication/problemGenerator"], function (problemGenerator) {
    describe('Derived problemGenerator for multiplication', function () {
        var lesson = {
        ...
        it('has the proper choice ranges', function () {
            expect(generator.choiceRanges[0].min).toEqual(lesson.min_multiplier);
            expect(generator.choiceRanges[0].max).toEqual(lesson.max_multiplier);
            expect(generator.choiceRanges[1].min).toEqual(lesson.min_multiplicand);
            expect(generator.choiceRanges[1].max).toEqual(lesson.max_multiplicand);
            expect(generator.choiceRanges[2].min).toEqual(lesson.min_multiplier * lesson.min_multiplicand);
            expect(generator.choiceRanges[2].max).toEqual(lesson.max_multiplier * lesson.max_multiplicand);
        });
        ...
        it('covers all permutations with permutations()', function () {
            ...
            for (i = lesson.min_multiplier; i < lesson.max_multiplier; i++) {
                for (k = lesson.min_multiplicand; k < lesson.max_multiplicand; k++) {
                    product = i * k;
                    // Equation limits check
                    if (product < lesson.max_answer && (product >= 0 || lesson.allow_negatives)) {
                        expectedPermutations.push([i, k, product]);
                    }
                }
            }
