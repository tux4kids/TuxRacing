...
require(["addition/problemGenerator"], function (problemGenerator) {
    describe('Derived problemGenerator for addition', function () {
        var lesson = {
        ...
        it('has the proper choice ranges', function () {
            expect(generator.choiceRanges[0].min).toEqual(lesson.min_augend);
            expect(generator.choiceRanges[0].max).toEqual(lesson.max_augend);
            expect(generator.choiceRanges[1].min).toEqual(lesson.min_addend);
            expect(generator.choiceRanges[1].max).toEqual(lesson.max_addend);
            expect(generator.choiceRanges[2].min).toEqual(lesson.min_augend + lesson.min_addend);
            expect(generator.choiceRanges[2].max).toEqual(lesson.max_augend + lesson.max_addend);
        });
        ...
        it('covers all permutations with permutations()', function () {
            ...
            for (i = lesson.min_augend; i < lesson.max_augend; i++) {
                for (k = lesson.min_addend; k < lesson.max_addend; k++) {
                    sum = i + k;
                    // Equation limits check
                    if (sum < lesson.max_answer && (sum >= 0 || lesson.allow_negatives)) {
                        expectedPermutations.push([i, k, sum]);
                    }
                }
            }
